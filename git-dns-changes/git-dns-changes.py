...
# For Eg: export cloudflare_email_id="xyz@uvw.com"
...
# Record types
type_records = ['SOA','A','TXT','NS','CNAME','MX','NAPTR','PTR','SRV','SPF','AAAA','CAA', 'ANAME']
...
# Checking for wrong entries and if there is wrong entries the script 
# will exit by displaying the wrong entries
wrong_entries = []
for rw in add_list:
  if rw[1] not in type_records:
      wrong_entries.append(rw)
if len(wrong_entries) >= 1:
    print("\nWrong entries.., please correct them and commit again")
    for record in wrong_entries:
      print(record)
    sys.exit()
...
for rw in complete_list:
  ...
  # Skipping if there is any wrong entries to remove
  if rw[1] not in type_records:
    continue
  ...
  # Skipping if it is a ANAME record
  if record_type == 'ANAME':
      print("ANAME record, skipping..")
      continue
  
  # Skipping if there is wrong name section
  try:
